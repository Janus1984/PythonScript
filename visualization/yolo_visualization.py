...
def xywhn2xyxy(box, size):
    ...
    xmin = (box[0] - box[2] / 2.0) * size[0]
    ymin = (box[1] - box[3] / 2.0) * size[1]
    xmax = (box[0] + box[2] / 2.0) * size[0]
    ymax = (box[1] + box[3] / 2.0) * size[1]
...
def draw_box(img, objects, draw=True):
    for object in objects:
        ...
        if draw:
            ...
            cv2.putText(
                img, category_name, (xmin, ymin), cv2.FONT_HERSHEY_SIMPLEX, 1, color
            )
    return img
...
def show_image(image_path, anno_path, show=False, plot_image=False):
    ...
    assert os.path.exists(anno_path), "annotation path:{} does not exists".format(
        anno_path
    )
    anno_file_list = [
        os.path.join(anno_path, file)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-ip", "--image-path", type=str, default="./data/images", help="image path"
    )
    parser.add_argument(
        "-ap",
        "--anno-path",
